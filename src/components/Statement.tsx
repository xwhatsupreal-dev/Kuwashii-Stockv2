...
export const Statement = ({ currentUser, onLoginClick }: { currentUser: any, onLoginClick: () => void }) => {
  ...
  useEffect(() => {
    if (!currentUser) {
      setLoading(false);
      onLoginClick(); // Auto-open login modal
      return;
    }
  ...
  if (!currentUser) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] px-4 space-y-6">
        <div className="w-16 h-16 bg-blue-500/20 rounded-xlflex items-center justify-center mb-2">
          <svg className="w-8 h-8 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColow">
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6 flex flex-col pt-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 relative overflow-hidden">
          <div className="relative z-10">
            <p className="text-zinc-400 text-sm font-medium mb-1">ยอดเงินงงเหลือ</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
          ...
          <div className="overflow-y-auto p-4 space-y-3 flex-1">
            {topups.length > 0 ? topups.map((t) => (
              <div key={t.id} className="flex justify-between items-center p-3 bg-zinc-800/50 rounded-xl">
                <div>
                  <p className="text-sm font-bold text-emerald-400">+ {t.amount} บาธ</p>
              ...
              <p className="text-center text-zinc-500 text-sm py-10">็ม่มีประวัติการเติมเงิน</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col h-[400px]">
          <div className="p-4 border-b border-zinc-800 bg-zinc-900 flex items-center justify-between">
            <h3 className="font-bold text-white flex items-center gap-2">
              <svg className="w-5 h-5 text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColow">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4" />
              </svg>
              ประวัติการซื้อสินง้าล่าสุ�T
            </h3>
            ...
            {purchases.length > 0 ? purchases.map((p) => (
              <div key={p.id} className="flex justify-between items-center p-3 bg-zinc-800/50 rounded-xl">
                ...
                <div className="shrink-0 text-right pl-2">
                  <p className="text-sm font-bold text-rose-400">-{p.price} บาด</p>
                  {p.qty > 1 && <p className="text-xs text-zinc-500">จำนว�: {p.qty}</p>}
                </div>
              </div>
            )) : (
              <p className="text-center text-zinc-500 text-sm py-10">็ม่มีประวัติการสั่งซื้อ</p>
            )}
          </div>
        </div>
      </div>
